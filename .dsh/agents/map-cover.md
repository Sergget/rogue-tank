...
## Key Systems
1. **Cover & Terrain tiers** (from `RULES.coverTiers`; tiers = half / full / bush / tree / fallen / soft / barricade / stump / rubble / water / river / mud / rock / intact / ruined / bridge). Each tier has 6 schema attributes: `passability` (movement factor; 0 = impassable tank push-out like water/river/rock; 0.4 = mud; 1.0 = passable), `shellBlock` (true = solid blocks shells; false = shells fly over like water/river/mud; 'grad' = graduated probability), `exposureProfile`, `destructible`, `drawStyle`, `tierGroup`. Legacy fields `mode`/`move`/`hp` are derived. Water/river allow shells to fly over while strictly blocking tank movement; rock blocks both shells and movement.
