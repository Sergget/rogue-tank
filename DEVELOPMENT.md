...
**半高掩体（`half`）**：简化版 3 条规则模型（2026-08-08 定型，取代连续高度切片与渐变）：

1. **暴露与射击**：
   - **炮塔**：100% 露出（不受半高掩体阻挡），炮弹可自由越过半高掩体射击。
   - **车体**：中坦车体 100% 被阻挡（0% 露出）；重坦车体露出 25%（75% 被阻挡）。
2. **距离压制（攻击方-被攻击方）**：取离【被攻击方】最近的一座半高掩体 $C_{near}$。若 $dist(攻击方, C_{near}) < dist(被攻击方, C_{near})$，说明攻击方离掩体更近，被攻击方视为**无掩体**（Exposure = 1.0）。若有多座掩体，以离被攻击方最近的一座为准。
3. **通行门控（driveBy）**：半高掩体按车型决定能否开过——**重坦可压过**（不毁、不推）、**中坦被挡**（MTV 推出）。
4. **方向判据（cutoff）**：掩体必须被实际弹道射线**在命中目标车体前完整穿过**（掩体出口距离 < 命中距离）才参与遮挡；骑上/压入掩体的坦克**不会**获得全方向遮蔽。
...
|---|---|---|---|---|---|
| 半高掩体 | `half` | 简化 3 规则遮挡（打炮塔 100% 露；打中坦车体 100% 挡，打重坦车体 25% 露；攻击方更近则目标无掩体，见 2.5） | 0.4 减速通行；**重坦可压过、中坦被挡推出**（driveBy） | 不遮 | ∞ |
...
- **坦克碰撞（Jitter 鬼畜消除）**：`resolveTankCollisions` 的 OBB MTV 推开逻辑经过了数学上的精准推开向量化重构，对碰撞法向速度进行了投影摩擦阻尼计算。两车对撞、推挤时能够极其平滑地阻隔并滑行，彻底消除了极速前后高频振荡、卡入和“鬼畜”的问题。
- **自定义炮塔本地旋转轴**：在设计器 `tank_designer.html` 侧边栏新增了 `turret.axis` dx/dy 自定义数字输入控件，打通了向 `tank_list.json` 中 round-trip 的无损存取，使设计师可以彻底、精确地微调座圈圆心与炮塔自转中心的关系（避免甩尾）。
