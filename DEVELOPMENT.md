...
- **炮塔自身旋转中心（axis）可自由编辑**：设计器侧栏 `t-axisDx`/`t-axisDy` 数字输入 + 『炮塔』模式拖拽十字/青点 **dx/dy 双向**自由调整（`turretDrawCenter()` = 座圈圆心 − axis，座圈不动、炮塔多边形相对其实时滑移）；『旋转中心』模式单独拖 pivot（锁车体中轴、仅 dx）。「炮塔自身居中对齐」一键把 axis 拉齐到包围盒中心（`axisDevReadout` 实时显示甩尾距离 = axis 与 bbox 中心偏差）。`buildExport` 写**真实 axis**、`applyTankData` 原样导入（不做 -axis 平移）→ round-trip 恒零漂移（Node 5 轮逐顶点验证，`verify-axis-rt`：axis=(6,-3) 存取逐位一致）；运行时 `applyTankConfig`（`js/tank_model.js`）仍按 `-axis` 平移顶点保证"局部原点=旋转轴"（`ARCHIVE.md` #1），MVP/compare 实弹渲染一致。本实现取代 `ARCHIVE.md` #11 旧结论（删输入+导出恒 0 归一化不再维护）。
- **炮塔/炮管对齐**：炮塔有两个独立设置——`turret.axis`（炮塔自身旋转轴，在炮塔自己的编辑坐标帧内，缺省 `(0,0)`，可在设计器自由编辑，见上一条）与 `turret.pivot`（该轴锚到车体的位置）。载入时把顶点按 `-axis` 归一化，保证"局部原点=旋转轴"这一不变量；炮管根部与炮塔共用同一旋转轴，杜绝"炮管悬空"与"旋转轴漂移到炮塔尾部"（见 `ARCHIVE.md` #1）。
