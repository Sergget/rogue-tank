...
- 难度完全靠敌人数量、敌人策略（AI 复杂度）、数值强度三者随节点推进同步提升；曲线定表 `RULES.difficulty`（diff = 0.15 + 0.8·t^1.25）。（2026-08-24 全面落地：entityMults 十维属性分化 + AI tierProfiles 行为分层，玩家隔离）。敌人难度现已采用弱→强曲线：RULES.difficulty.entityMults 十维属性表下界<1（简单节点敌人更弱更慢）、上界略>1（困难节点更强更快）；普通敌人输出端封顶采用四键方案（2026-08-25 起，取代旧 enemyStatCapVsPlayer=0.8；#A16 于 2026-08-27 收口）：penCapVsPlayer=1.2（穿深封顶）/ dmgFloorVsPlayer=0.4（伤害地板）/ dmgCapAmmoMult=0.7（最强弹种终伤天花板）/ speedVsPlayer={baseFloor:0.3, baseCeil:0.6, randMin:0.85, randMax:1.15}（极速相对玩家公式 lerp×rand×player.maxSpeed）。**三处封顶/地板/速度已于 #A16 改为经 addModifier 注入（scope 'run'、source 'difficulty-cap'）而非直写 t.stats.*，防后续 refreshStats 把直写值抹回未封顶**；换算纯函数 js/tank_model.js difficultyCapMuls 把「相对玩家的目标绝对值」转为「相对敌人 t.base.X 的 mult 系数」，与先行的 entityMults（source 'difficulty'）加法聚合兼容、逐值等价于旧直写。Boss 经 applyDifficultyMults 第三参（applyPlayerCap=false）跳过此封顶。敌军生成改为聚集布点（RULES.nodeMap.enemyClusterRadius/SizeMin/Max/CountBase）：每聚集点 2–5 辆、聚集点数随难度提升。
...
- 节点间开放卡牌三选一与**局内商店**（已落地，2026-08-24；D3 #A1/#A3 重构定型，2026-08-26）：局内商店按**当前得分**计价消费，只售 run 内属性升级（modifiers scope:'run'，本局结束清除、不带出存档），消费独立记账、**不减损**终局转化用的累计得分；**永久升级商店**消费由终局累计得分 ×10% 转化的点数。两套商店货币互不流通。商品表 `RUN_SHOP_DEFS`（#A1/#A3 后 12 项四分组——火力 5：快速装填/穿深加工/火力增强/精密火控/姿态稳定；防护打包 2：车体装甲包/炮塔装甲包（各三面 +2mm）；机动 2：引擎超压/马力强化；杂项 3：紧急维修/修理箱速冷/医疗包速冷）。定价 `round(baseCost×costGrowth^level)`，可重复购买商品 costGrowth ≥1.5、无上限商品以 maxLevel:99 表示。**硬边界达限拒购**：装填下限 0.5s / 极速上限 375px/s(150km/h)，购买后穿越 `def.limit` 边界时按钮禁用（`runShopLimitBlocked` 纯函数，数值对照 RULES.parameterLimits 同源）；极速 desc/预览统一 km/h 口径。**散布通道解耦**：姿态稳定改挂独立运动系数 stat `motionSpreadMul`（mult ×0.85，computeStats 默认继承出厂 base.spreadMult 并钳 ≥ spread.multFloor）——运动三源散布与精度基准 spreadMult 彻底解耦，motionSigma 消费 `stats.motionSpreadMul ?? stats.spreadMult`（旧快照向后兼容）；维修/医疗冷却显式钳底 15s。**可升级次数由具体数值决定（2026-08-28 定案，取代写死 maxLevel）**：有自然数值边界的商品改以 `runShopLimitBlocked` 按 `RULES.parameterLimits` 动态判定达限——`fast_reload`(reload.min 0.5) / `engine_overdrive`(maxSpeed.max 375) 走显式 `limit` 字段、`precision_gunnery`(spreadMult.min 0.5) 与 `steady_mount`(motionSpreadMul.min 0.5，本轮新增与 spreadMult 同级边界) 补 `limit` 字段、多面打包装甲包（hull/turret 三面）在无 `limit` 时由 `runShopLimitBlocked` 逐面按 `parameterLimits.armor.*` 推导达限（读 `player.stats` 真实各面值，非单一 curVal）；上述商品的 `maxLevel` 统一为 99 作**防御性兜底**（真正上限由数值决定，卡牌叠加压低数值时可购级数随之动态缩小）。`computeStats` 的 `multFloor` 物理钳底保持不动（防穿零最后保险）。局内商店除结算/奖励节点外，现也可从 ESC 暂停菜单的「局内商店」按钮进入；暂停面板内商店按钮此前因 pointer-events 失效不可点击，已修复。
...
- **模块概率分区与参数极限制（2026-08-26 落地，P-49 核心批）**：
  ...
  - **功重比显示**：设计器 / 对比器新增功重比读数。
   - **回放基线重锚**：28f3e684 → 1e49b3fc → 5b8c4126（#A17 零开火修复）→ 799b65f（P-43 敌群构成）→ 5d754f53（#A18 回放代理玩家补瞄准+开火，超时率 46.5%→15.5%）→ **49a6b2c9**（当前值，P-46 类别化敌军：dummy 数值启发式推导为 medium 类，行为覆盖由 heavy moveLock 转为 medium 基线；test-replay.js 仅校验确定性不钉常量）。
...
- **活跃开发项（2026-08-26 起）**：近期计划见 docs/PLAN.md：玩法线 PLAN 已全部完成归档（P-34~P-41）；当前着力**玩法核心专项 P-42~P-44**（卡牌平衡审计 P-42 扩展维度已完成无 red 警告 / 地图生成质量 P-43 5 模板难度校准与 D5 半高禁令落地 / 战斗与 AI 修补 P-44 确定性模拟器与友军击杀五折记分已落地）；视觉专项（旧 P-42~P-45）与音频专项（旧 P-46~P-49）暂缓，规格全文移交 `.opencode/agents/asset-artist.md` / `sound-designer.md`「暂缓储备规范」章节维护；候选库前移为 P-45。当前待处理问题见 docs/ISSUES.md：#78（剩余范围=设计器 verts UI）、#A9（半高掩体低生效评估）、#A11（地形占位冲突，并入 P-43）。下方历史记录中的 P-27/P-29/P-30 为旧编号（已完成工作），勿与本批混淆。
