...
10. **2026-09-16 炮塔跨节点前移根治 + 路网重做已落地并验证**（见 §4.18，关闭 ISSUES #B6/#B7，用户反馈三条）：① 炮塔前移双根因——`tank_boss.js` 原地 `*=scale` 污染 `tankListData` 共享 spec（Boss 每节点 ×2 雪球：pivot 7→56 / anchors 32→256）已改为拷贝/整体替换，W3「节点推进炮塔前移」有意特性按用户裁定整体删除（`RULES.progress` + `tank_map.js` 两函数 + mvp 调用点 + `test-rework-w3.js`）；② 路网重做——取消逐段跳段（截断根因，缺口 202~246px→0）、端点落节点边界 + `lineCap:'butt'`（圆弧路头根因）、抑制浅角互穿（夹角 35°→近似直角）。全部模板连通性升至 1.000。`npm run check` / `npm test` / `npm run test:browser` 三链全绿。
11. **2026-09-16 路网 v2（用户二次反馈）已落地并验证**（见 §4.18 #B7 v2，规范 specs/map.md §10.2）：① **路口感**——v1 逐链各描三遍致「两条路机械叠加」，改为两遍绘制（先合并路面、再统一画标线并**在路口挖空**），新增 `roadJunctions` 经 `makeNode` 平移后驱动 `bakeNodeGroundLayer` 断线；② **拓扑多样化**——v1 恒为同一十字（实测 70/84 张，且主轴绑定地图长边故 7 模板全横向）→ 改为 6 种拓扑抽取（贯通/十字/单侧T/双侧T/错位平行/错位丁字对），T 形支道锚定干道真实折线（修掉猜测锚点造成的 118 处缺口），路口数上界 2 且覆盖 0/1/2。顺带修正泥斑断言未扣模板自带块。`npm run check` / `npm test` 全绿。
