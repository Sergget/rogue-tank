...
#### 3.24 模块代码质量与架构缺陷修复（2026-08-22 会话，ISSUES #61~#74，重做核实已完成）

以下 14 项共享模块代码质量与物理/逻辑缺陷经排查修复并完全通过验证（其中 #61~#65、#70~#72、#74 经重新实现并补齐回归测试用例，全量单测 exit 0 通过）：
...
> **收尾核实（2026-08-22 #75 闭环）**：上述 14 项经 `npm run check` + `npm test` 全绿复核，结论已全部收口至本节且独立可读（不依赖已删 ISSUES 条目）；`docs/ISSUES.md` #75“归档条目 #61~#74 的修复大部分缺失于工作区”于同日按 4 步生命周期删除并归档至 `ARCHIVE.md`（28 行原文），§6 排序不受影响（均为已归档条目重做）。
...
#### 3.26 战斗核心管线收敛（P-28，2026-08-23 会话；设计见 §2.5/§2.6）

- **定型设计**：战斗发射/命中判决管线唯一来源收敛到 `js/tank_fire.js`（~376 行 / 24783 字节，纯逻辑 ctx 显式注入），收编 `fireTank`/`fireSmokeShell`/`tryFire`/`tryFireSmoke`/`computeSolution`/`updateSolution`/`stepShells` + `shellVerticalDecision`；依赖经 ctx 显式传入（`covers`/`entities`/`fx`/`audio`/`RULES` 钩子），浏览器端按全局脚本顺序加载、底部 `module.exports` 兼容 Node 测试。**语义守恒**：半高越掩高度插值、护盾吸收守卫（`!s.absorbed`）、HE 破障/溅射、烟幕阻断视线不阻弹道、二次跳弹禁止均保持一致（修复 mvp/bench 分叉漂移，如 bench HE 破障缺 `!s.absorbed` 守卫）。
- **页面侧**：`tank_mvp.html` / `tank_bench.html` 均改为 `<script src="js/tank_fire.js">` 并删除双份内联副本（git diff 811 deletions：mvp −483 行、bench −335 行），仅保留输入/HUD/DOM 胶水；内联残留 `shellVerticalDecision` 为薄包装存根（mvp L1534 / bench L557，已迁移至模块），`stepShells` 调用改为模块委托 `stepShells(dt,{worldW,worldH})`。
- **脚本加载顺序**：`js/tank_rules.js` 最先 → `js/tank_utils.js` → … → `js/tank_drone.js` → **`js/tank_fire.js`** 紧随其后（`tank_mvp.html` L402 / `tank_bench.html` L228 同位）→ 页面内联脚本；新增模块 `types/globals.d.ts` 与 `package.json` 脚本清单已同步。
- **验证与测试**：新增 `scripts/test-fire.js`（7826 字节，9 项检查：`shellVerticalDecision` exposure / `fireTank` HEAT/HE/absorbed/bounce 等）全部通过；`npm run check` 全绿（含 `js/tank_fire.js` + `tank_mvp.html#inline0` + `tank_bench.html#inline0` + `scripts/test-fire.js`，`tsc --noEmit` 通过）、`npm test` 全绿（All fire checks passed.）。后续任何弹道/掩体判决改动只需改 `js/tank_fire.js` 单点，无需双页双改。
...
27. **卡牌 × Loadout 衔接（遗留衔接点，来自 M10）**：卡牌 `effect.type==='ammo'` 的弹种改造需与出战 `ammoLoadout` 键集求交——改造指向**未配备**弹种的改造卡不应产生可切换目标（或提示先装备该弹种）；方案待定后实现。**前置阻塞已解除（P-28 已于 2026-08-23 完成，战斗管线已收敛至 `js/tank_fire.js` 单点，见 §3.26）**。
...
29. ~~**战斗核心管线解耦（P-28）**~~ — **已完成（2026-08-23，P-28，见 §3.26）**：`tank_mvp.html` 与 `tank_bench.html` 的战斗发射/命中判决管线已收敛到 `js/tank_fire.js`（~376 行，`fireTank`/`tryFire`/`shellVerticalDecision`/`updateSolution`/`stepShells` + 飞行积分；`!s.absorbed` 等漂移已修复；`tank_mvp.html` L402 / `tank_bench.html` L228 紧随 `tank_drone.js` 加载），后续弹道/掩体改动单点生效。原前置阻塞条目 27 已解除。
...
31. **文档分卷与 agent 纪律（P-30，见 PLAN.md；约定已先行生效）**：2026-08-22 复盘证实巨型单体文档（ARCHIVE ~165KB / DEVELOPMENT ~170KB / npm test 输出 ~120KB+）的全文反复读取是子代理死循环的系统性根因。**已先行落地**：`AGENTS.md §3.5`「大文档 grep 优先、禁全文读取、同文件重读≤3 次」工作流约定；`.opencode/agents/test-runner.md`（各命令至多一轮 + 调用上限 12 + 允许部分报告）与 `docs-agent.md`（bash=deny 零 shell 依赖 + 切片读取 + 上限 15）硬性纪律；`.opencode/agents/orchestrator.md` Dispatch Discipline 四条。**待办（P-30）**：ARCHIVE 正文按月分卷至 `docs/archive/<yyyy-mm>.md`，索引表留主文件，于再增 ~50KB 时执行。
