...
| 2026-08-23 | `PLAN.md` | A-1. 更新 AGENTS.md 模块清单 | 已完成并验证（补录 `tank_fire.js`/`tank_screens.js` 到共享模块表，结论见 AGENTS.md §1） |
| 2026-08-23 | `PLAN.md` | A-2. 清理 AGENTS.md §5 技术债列表 | 已完成并验证（删已完成项 P-12/P-08/P-10/P-11/M10，仅保留 P-21/23/24/25/26，结论见 AGENTS.md §5） |
...
### [2026-08-26] 归档自 docs/ISSUES.md #A1/#A3 平衡与系统接线问题（已修复，共 2 条）

> 来源文档：`docs/ISSUES.md`「平衡与系统接线问题（2026-08-26，#A1~#A16）」条目 **#A1** 与 **#A3**；删除日期 2026-08-26。已按用户既定裁定实现并验证（npm run check / npm test / test:browser 全绿，replay-baseline hash `1e49b3fc` 不变）。结论去向：#A1 → `docs/specs/combat.md` §2「运动散布与精度基准解耦」+ `docs/DEVELOPMENT.md` §2.1 局内商店段；#A3 同段（RUN_SHOP_DEFS 12 项四分组重构 + limit 达限拒购 + km/h 口径 + 冷却钳底 15s）。其余 #A 条目保持待处理不动。以下为原文。
...
### [2026-08-26] 归档自 docs/ISSUES.md #A15 成员防护内衬两张卡完全未实现（已修复，1 条）

> 来源文档：`docs/ISSUES.md`「平衡与系统接线问题（2026-08-26，#A1~#A16）」条目 **#A15**；删除日期 2026-08-26。已修复并验证：`js/tank_physics.js` 经 `passiveValues(target,'spall_liner')` 取多来源最小值 `spallMul`，在 `applyModuleDamage` 乘入最终模块/乘员伤害；活浏览器实测 `giveCard('support_spall_liner')` 后敌方 PEN 伤害均值降至无内衬 0.7981 倍（预期 0.8），多来源取最强语义正确；epic 卡 `spall_liner.json` 当前 value 0.85。结论同步至 `docs/specs/combat.md` §2 模块节。以下为原文。
...
### [2026-08-26] 归档自 docs/ISSUES.md #A17 批量 seed 回放出现高比例零开火节点（已修复，1 条）

> 来源文档：`docs/ISSUES.md`「平衡与系统接线问题（2026-08-26，#A1~#A17）」条目 **#A17**；删除日期 2026-08-26。已在 `js/tank_cover.js`/`js/tank_nodegen.js`/`js/tank_ai.js` 落地生成期 LoS 走廊 + 运行期侧向绕行并经三链验证（npm run check / npm test / test:browser 全绿），零开火节点 9/40→7/40。以下为原文。

### #A17. 批量 seed 回放出现高比例零开火节点（P-44 收集）

**状态：** 处理中

**来源：** P-44 批量 seed 回放收集（2026-08-26）。方法：headless 全链回放 `node` 执行 `js/tank_sim.js` 的 `runReplay({seed, nodeCount:5, dt:1/30, maxNodeTime})`，seed 1~15（maxNode=40s）与 seed 1~6（maxNode=90s）两轮。

**可复现证据（定量）：**
- seed 1~15 / maxNode=40s：75 个节点中 **18 个（24%）零开火**（playerShots+enemyShots=0），结局 timeout 占 46/75（61%）。
- seed 1~6 / maxNode=90s：30 节点中仍 **5 个（17%）零开火**，timeout 降至 12/30（40%）。
- 21 局全程**无任何异常/崩溃**，15/15 seed 摘要互异（确定性正常）。
- 关键定性：`tank_sim.js` 中玩家与敌军**均由 `aiDecide` 驱动**（L316-349，玩家同样调 `fireTank`，line 349），故"双方零开火"非 sim 玩家假象，而是真实交战未触发。

**复现命令（简述）：**
- `node` 执行 `js/tank_sim.js` 的 `runReplay({seed:i, nodeCount:5, dt:1/30, maxNodeTime:40})` 遍历 i=1..15；
- 同法 `maxNodeTime:90` 遍历 i=1..6；
- 汇总每节点 `playerShots+enemyShots` 与结局（timeout/kill/win）。

**影响：** 约 1/5~1/4 节点全程无交火、以 timeout 收场，浪费玩家时间且结算信号失真。

**根因（2026-08-26 @enemy-ai 排查）：**
- 主因：零开火节点视线（LoS）被掩体**永久遮挡**，而 AI 无绕行寻路，双方直线卡在遮挡体两侧 → 开火门控的 LoS 条件恒假。
- 铁证：21 局 105 节点中 23 个零开火节点**全部 `losFalseFrac=1.00`**（视线恒假），正常节点该值 0.0~0.94，证明 `hasLineOfSight` 本身正常，纯几何遮挡。
- 反证（排除"敌军不靠近"）：临时 patch 强制未接战敌军朝玩家机动，零开火**不降反升**（18→21/75），靠近也救不了，LoS 才是真瓶颈。
- 非主因（已排除）：激活触发距离 / `engageRange`（双方均可进 engage 范围 520×难度比，minDist 49~388，仅因 los=false 不开火）。
- 关键代码点：`tank_ai.js:240` 敌军开火需 `los`；`tank_sim.js:345-347` 玩家开火二次重检 `COVER.hasLineOfSight` 亦恒假；`tank_ai.js:176-182` 未接战早退 `move=0`（叠加放大）；`tank_map.js:192 / :223-229` 生成玩家置左缘 `0.10w`、敌簇聚右 2/3，中间横亘 full/half 掩体，AI 与 sim 玩家均直线机动（`tank_ai.js:218` / `tank_sim.js:326-330`）撞墙无侧向绕行 → 永久卡死。

**推荐修复（未实施）：**
1. 【低/中】节点生成保证可达 LoS 走廊（`tank_map.js` makeNode + generateNode）：几何校验，若玩家出生点与任一敌簇质心连线被 full 掩体完全遮挡，则开缺口/挪簇位/降遮挡 tier，确保至少一条直视线。
2. 【中】LoS 被挡分支加侧向绕行转向（`tank_ai.js:203-224` search 分支当前直冲 `desired`，改为取遮挡体法线做垂直分量转向恢复 LoS）。
3. 【中/高】放宽开火 LoS 门控为"已知目标记忆即盲射"（`tank_ai.js:240` 改 `los || lastKnownPlayerPos`，需配 `alertEntity` 已设置的记忆点）。
4. 【低】sim 玩家补简易绕障（仅回放基线噪声，不治本）。
- 优先建议 **1+2 组合**（生成期保证可达 + 运行时绕行），从根消除卡墙零开火。

**处理方向：** 根因与推荐修复已见上，原"建议 @enemy-ai 核实"已闭环；条目保持 `处理中`（修复未落地）。

---

### #A16. 敌方/Boss 参数绑定审计结论 + 配套发现（2026-08-27 归档，原状态：待处理 → 已解决）
...
**处理方向（原）：** a-3 速度公式提为 RULES.difficulty.speedVsPlayer 表并消除与 entityMults 打架；直写改经 modifiers 注入防 refreshStats 回归；敌 pen 浮动按上述落点实施；修 DEVELOPMENT.md:25。

**修复落地（2026-08-27，经 A→C 自动化流程实施）：**
- 三处直写（pen 封顶 / dmg 地板+天花板 / maxSpeed 公式）由 `tank_mvp.html` `applyDifficultyMults` 改为经 `addModifier` 注入（scope 'run'、source 'difficulty-cap'），不再被后续 refreshStats 抹回未封顶值；
- 抽出纯函数 `js/tank_model.js difficultyCapMuls(t, opts)`：把「相对玩家的目标绝对值」换算为「相对敌人 `t.base.X` 的 mult 系数」`mul=(target−t.stats.X)/t.base.X+1`，与先行的 `entityMults`（source 'difficulty'）加法聚合兼容（computeStats 对同 stat 多 mult 聚合为 `1+Σ(v_i−1)`），**逐值等价于旧直写**；
- maxSpeed 页内硬编码 lerp 收口进 `RULES.difficulty.speedVsPlayer={baseFloor:0.3, baseCeil:0.6, randMin:0.85, randMax:1.15}`，消除与 entityMults.maxSpeed 两套机制打架；
- 回归断言落 `scripts/test-modifiers.js` §21（4 场景：正常区间只注 speedMul / pen 超限 / dmg 低于地板 / dmg 高于天花板，均断言注入后 stats 逐值等价旧直写目标）；
- `docs/DEVELOPMENT.md` §2.1 已同步「四键方案」描述（penCapVsPlayer / dmgFloorVsPlayer / dmgCapAmmoMult / speedVsPlayer）；
- 回放基线 hash 不受影响（tank_sim.js 的 applyDifficulty 不应用玩家封顶逻辑，验证 `npm test` 全绿、seed=1 hash 仍为 799b65f）。

**遗留（未改，保留原审计结论）：** 敌 pen 开火时刻正态浮动落点（roll 0.6–1.4）仍待 P-44/P-46 实施；Boss tuning 链零玩家引用本就正确，无需改。

---

### [2026-08-27] 归档自 docs/ISSUES.md #A14 "全线高爆战术"过强 / "超口径高爆弹"未生效死效果（已修复并验证）

> 来源文档：`docs/ISSUES.md`「平衡与系统接线问题（2026-08-26，#A1~#A16）」条目 **#A14**；删除日期 2026-08-27。已于 2026-08-26 在 `2d139bc feat(cards+shop): #A14/#A15卡牌接线` 落地并走完三链验证（此前 ISSUES 条目因遗漏未在当批归档，本批补走 4 步生命周期）。结论去向：`docs/specs/cards.md` §6「被动卡接线结论」+ `docs/PLAN.md` P-42 追加范围。以下为原文。
...
**修复落地（2026-08-26）：**
- `demo_all_he_doctrine.json` 移除全局 reload×0.85，仅保留 HE dmg×1.2 + HE pen×1.2 两重弹种效果（#A14a）；
- `demo_overmatch_shell.json` 移除 HE dmg 效果、保留 passive overmatch 0.85，按用户意向转 AP（#A14b）；
- `js/tank_physics.js` 新增 `passiveValues()` 统一消费 + resolveHit 内 overmatch 口径碾压分支（AP/APCR 命中 eff ≤ pen×0.85 时跳过跳弹/过陡 BLOCK、强制穿透路径，res.overmatch=true 标记）；
- 断言落 `scripts/test-cards.js`（#A14a 移除 reload / #A14b overmatch 免跳弹 + 阈值不满足仍跳弹 + HEAT 不受影响）。

---

### [2026-08-27] 归档自 docs/ISSUES.md #A10 树冠视觉尺寸与逻辑盒脱节 + 倒树不可辨识（已修复并验证）

> 来源文档：`docs/ISSUES.md`「平衡与系统接线问题（2026-08-26，#A1~#A16）」条目 **#A10**；删除日期 2026-08-27。结论去向：`docs/specs/map.md` §4「贴图资产与烘焙层」+ `js/tank_assets.js` `tree.bakeCanopy`。以下为原文。
...
**修复落地（2026-08-27）：**
- `js/tank_assets.js` `tree.bakeCanopy` 冠径乘数 `Math.max(w,h)*1.9` → `*1.4`，使树冠视觉半径更贴近逻辑 OBB（24×18），缓解"所见远大于所挡"的脱节；倒树（fallen）保留根部伐倒断面高亮（既有实现已含根部圆截面 + 年轮描边）。
- `npm run check` 全绿（tank_assets.js 模块加载与语法校验通过）；纯视觉改动，无自动化断言（视觉需浏览器人工核对）。
---

### [2026-08-28] 归档自 docs/ISSUES.md #A5 自身模块受损/成员受伤无 UI 指示（已修复并验证）


> 来源文档：`docs/ISSUES.md`「平衡与系统接线问题（2026-08-26，#A1~#A16）」条目 **#A5**；删除日期 2026-08-28。结论去向：`docs/specs/combat.md` §2 模块节「成员/模块受损状态条」。以下为原文。
...
- `tank_mvp.html` 顶部中央新增 `#moduleStatus` 状态条 + `updateModuleStatus()`（每帧刷新，挂入 `updateHud()`）：读 `player.debuffs`（gunner/loader/driver/commander/engine/ammo/breech 中文药丸 + 剩余秒）与 `player.trackBroken`/`immobT`（履带断裂），无受伤隐藏。

- 验证：`npm run check` + `npm run test:browser` 全绿（mvp 无 console/page 错误）；纯视觉改动。

---

### [2026-08-28] 归档自 docs/ISSUES.md #A18 回放基线批量 seed 扫描：大量节点超时（已修复并验证）


> 来源文档：`docs/ISSUES.md`「平衡与系统接线问题（2026-08-26，#A1~#A18）」条目 **#A18**；删除日期 2026-08-28。结论去向：`docs/specs/combat.md` §5.1「回放高 timeout 根因与修复」。以下为原文。
...
- 诊断脚本 `scripts/diagnose-replay-seeds.js` 跑 `24 seeds × 4 nodes，maxNodeTime=45s`：win=18 / loss=17 / **timeout=61**（总 96 节点，超时率 ≈63%）；23/24 seed 达 timeoutHeavy 阈值。

- zeroFire=0 说明双方确有交火，但绝大多数节点分不出胜负——「有开火但不收敛」类，与 #A17「零开火卡墙」不同根因类。


**影响：** 高超时率使回放基线难以区分「真实平衡」与「sim 缺陷假超时」，削弱回归判据信度。


**处理方向（原）：** 排查 sim 代理玩家推进阈值与 AI 对峙；区分真平衡超时 vs sim 缺陷假超时；若是 sim 缺陷，增强代理玩家或加 forceResolve 兜底，使超时率降至可解释区间再重锚基线。


**修复落地（2026-08-28）：**

- 根因定位：`js/tank_sim.js` 代理玩家只推进位置、从不转炮塔（turret 恒 0）也不开火（fire 恒 false）；远处敌人因距离 > aiTriggerDist（默认 700~1120px）保持 patrol/engaged=false 不接战 → 双方僵持 0 开火假超时。

- 修复：代理玩家始终把 `out.turretDesired` 指向最近敌人，并在大致对准（≤~7°）时置 `out.fire=true`（范围/LoS/装填由开火块统一门控）；仅改 `tank_sim.js`（回放 harness，零正式游戏影响）。

- 实测（40 seeds × 5 nodes，maxNodeTime=60s）：超时率 46.5%→15.5%（93→31/200），win 45→140，timeoutHeavy 30→6 seed；回放 hash 重锚 799b65f→5d754f53（可归因）。

- 残留 15.5% 属第二层「敌人超触发距离不接战 + LoS 受阻不逼近」的真平衡/对峙问题，待对比真人实跑录像再裁定是否加强 AI 迫近或加 forceResolve（见 specs/combat.md §5.1）。
