...
// tank_audio.js — M1 声音占位系统：Web Audio 程序化合成 8 类占位音效。
//
...
//
// 浏览器全局（按序加载后可用）：
//   SOUND_DEFS          — 8 类音效参数表（fire/pen/block/bounce/ammoBlew/trackBreak/fireDOT/ui）
//   AUDIO_SETTINGS      — 音量分级（combatGain 战斗 / uiGain UI）
//   playSound(key, opts?) — 单入口播放（内部自动 ensureAudio；未知键/无 AudioContext 静默返回 false）
...
function _scheduleLayer(ctx, L, busGain, defGain, t0){
  ...
  if(L.kind === 'osc'){
    ...
    node.connect(env); env.connect(_master);
    src.start(t); src.stop(end + 0.02);
  }
}

// 单入口：播放一类音效。opts 可选 { gain: 倍率, delay: 秒 }；未知键或不可用环境返回 false。
function playSound(key, opts){
  ...
  for(const L of layers) _scheduleLayer(ctx, L, busGain, defGain * gMul, t0);
  return true;
}
